...
import { cancelDoseRemindersForMedication, cancelDoseSlotReminders } from './reminderScheduler'
...
/** Ensure array-shaped PHI fields are real arrays after DB/E2EE round-trips. */
function normalizeMedicationArrays(med: Medication): Medication {
  return {
    ...med,
    prn_amount_hints: asStringArray(med.prn_amount_hints),
    prn_symptom_hints: asStringArray(med.prn_symptom_hints),
  }
}

function normalizeDoseLogArrays(log: DoseLog): DoseLog {
  return {
    ...log,
    prn_symptoms: asStringArray(log.prn_symptoms),
  }
}
...
export async function markDoseTaken(
  ...
  // Clear today's pending alert for this slot immediately (before UI reschedule).
  await cancelDoseSlotReminders(medicationId, scheduleTime)
...
export async function redeemDose(
  ...
  if (!med || !isMedicationActiveOn(med as Parameters<typeof isMedicationActiveOn>[0], date)) {
